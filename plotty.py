...
class CountryData:
    ...
    Uses Plotly for interactive visualizations:
        - Click legend items to toggle countries
        - Hover tooltips
        - Zoom / pan
        - Export to HTML (PowerPoint-friendly)
    """
    ...
    def _add_trace(self, fig, df, x, y, y_scale=1):
        """
        Internal helper to add a line trace to a Plotly figure.
        """
        fig.add_trace(
            go.Scatter(
                x=df[x],
    ...
    def plot_gdp(self, fig):
        if self.gdp is None:
            self.get_gdp()
        self._add_trace(fig, self.gdp, "Date", "GDP", y_scale=1e6)

    def plot_gdp_per_capita(self, fig):
        if self.gdp_per_capita is None:
            self.get_gdp_per_capita()
        self._add_trace(fig, self.gdp_per_capita, "Date", "GDP_per_capita", y_scale=1e3)

    def plot_inflation(self, fig):
        if self.inflation is None:
            self.get_inflation()
        self._add_trace(fig, self.inflation, "TIME_PERIOD", "INFLATION_YOY_PCT")

    def plot_unemployment(self, fig):
        if self.unemployment is None:
            self.get_unemployment()
        self._add_trace(fig, self.unemployment, "Date", "UnemploymentRate")

    def plot_bond_yields(self, fig):
        if self.bond_yields is None:
            self.get_bond_yields()
        self._add_trace(fig, self.bond_yields, "Date", "BondYield")

    # -------------------------------
    # Plot multiple countries (HTML)
    # -------------------------------

    @classmethod
    def plot_multiple_html(cls, countries, indicator, output="chart.html"):
        """
        Plot multiple countries on one interactive Plotly chart
        and export to a self-contained HTML file.
        """
        ...
        for country in countries:
            c = cls(country)
            getattr(c, f"plot_{indicator}")(fig)
...
choice_map = {
    ...
    "5": ("Long-Term Bond Yields", "bond_yields")
}

print("Select an economic indicator to plot:")
for key, (label, _) in choice_map.items():
    print(f"{key}: {label}")

choice = input("Enter the number of your choice: ").strip()
indicator = choice_map[choice][1]

countries = input("Enter countries (comma-separated): ")
countries = [c.strip().title() for c in countries.split(",")]

CountryData.plot_multiple_html(
    countries=countries,
    indicator=indicator,
    output=f"{indicator}.html"
)
